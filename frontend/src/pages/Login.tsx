...
import { Eye, EyeOff, LogIn, Shield, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export default function Login() {
  ...
  // Auto-focus email and check for success messages from redirect
  useEffect(() => {
    if (location.state?.message) {
      setSuccessMsg(location.state.message);
      // Clear the state to prevent message persistence on refresh
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            {successMsg && (
              <motion.div
                ...
                role="status"
              >
                <CheckCircle2 size={15} style={{ color: '#10B981', flexShrink: 0, marginTop: '1px' }} aria-hidden />
                <p style={{ fontSize: '0.8125rem', color: '#047857' }}>{successMsg}</p>
              </motion.div>
            )}
